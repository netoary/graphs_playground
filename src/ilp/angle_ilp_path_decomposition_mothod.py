...
class AngleILPPathDecompositionMothod(AbstractIlpPathDecompositionMothod):
    ...
    def _init_x_variables(self):
        ...
        for v in vertices:
            ...
            for pair in iterator:
                angle = self.pair_to_angle([pair[0], pair[1]])
    ...
    def angle_to_common_vertex(self, angle):
        """
    ...
    def edge_to_other_vertex(self, edge, vertex):
        """
    ...
    def angle_to_other_edge(self, angle, edge):
        """
    ...
    def pair_to_other(self, pair, sample):
        """
            Retorna o outro par;
        """
        if sample == pair[0]:
    ...
    def mycallback(self, model, where):
        ...
        if where == GRB.Callback.MIPSOL:
            ...
            cycles, paths = self.solution_cycles(vals, 5, model._G)

            for c in cycles:
                model.cbLazy(self.cycle_constraint_extended(c, model._x) <= len(c) - 3)
            for path in paths:
                #aqui setei (2k+1) = 5
                model.cbLazy(self.path_constraint(path, model._x) <= 4)
            
            model.update()


    # DÌVIDA!!!
    def solution_cycles(self, solution, k, G):
        """
        ...
        for i in dic:
            if len(dic[i]) == 2 and dic[i][0] == False:
                dic[i][0] = True
                angle = dic[i][1]
                common_vertex = self.angle_to_common_vertex(angle)
                initial_vertex = self.edge_to_other_vertex(i, common_vertex)
                new_sequence = [initial_vertex, common_vertex]
                e = self.angle_to_other_edge(angle, i)
                while len(dic[e]) == 3:
                    dic[e][0] = True
                    new_vertex = self.edge_to_other_vertex(e, new_sequence[-1])
                    new_sequence.append(new_vertex)
                    pair = [dic[e][1], dic[e][2]]
                    angle = self.pair_to_other(pair, angle)
                    e = self.angle_to_other_edge(angle, e)
    ...
    def cycle_constraint_extended(self, cycle, w):
        """
        ...
        """
        angles = self.cycle_to_angles(cycle)
        constraint = 0
        for angle in angles:
    ...
    def path_constraint(self, vertices, w):
        """
        ...
        for i in range(0, l - 2):
            edge1 = self.pair_to_edge([vertices[i], vertices[i+1]])
            edge2 = self.pair_to_edge([vertices[i+1], vertices[i+2]])
            angle = self.pair_to_angle([edge1, edge2])
            constraint = constraint + w[angle]
        return constraint


    def pair_to_edge(self, pair):
        """
            Dado dois vértice, 
    ...
    def cycle_to_angles(self, cycle):
        """
            Dado um ciclo, 
        ...
        for i in range(0, len(cycle) - 2):
            ...
            v = cycle[i+1]
            w = cycle[i+2]
            edge1 = self.pair_to_edge([u, v])
            edge2 = self.pair_to_edge([v, w])
            angle = self.pair_to_angle([edge1, edge2])
            angles.append(angle)
        u = cycle[-2]
        v = cycle[0]
