...
from rl.utils import possibleMoves, takeHangingEdges, move, unmove, takeHangingEdges_v2, separa_cores_por_vertice, define_estado_atual, canonicalDecomposition, extractColor
...
class RegularDecompositionEnvironment(gym.Env):
    ...
    def _observation_space(self):
        ...
        invariantes por isomorfirmos: mais abaixo mais forte
        1) numero de arestas pendentes -> diferente os estados do grafo é diferente
        2) vetor de númeor de elementos de cada tipo [P, T1, T2, T3] -> P, 3 trilhas
        [0, 0, 0, 0]
        3) [10 entradas], um K4 com loop

        '''
        # states = MultiDiscrete([1000]*10)
        states = Box(low=-np.inf, high=np.inf, shape=(9, ), dtype=np.uint8) # MlpPolicy
        # states = Dict(trails=Box(low=-np.inf, high=np.inf, shape=(10, ), dtype=np.uint8)) # MultiInputPolicy
        return states
    ...
    def _show(self):
        v_color_dic = separa_cores_por_vertice(self.possible_moves)
        proximo_estado = tuple(define_estado_atual(self.current_graph, v_color_dic))
        # print("*"*100)
    ...
    def _get_color_obs_position(self, trails):
        mapping = {
            '01': 0,
            '10': 0,
            '02': 1,
    ...
    def _get_obs(self):
        ...
        for colors in vertex_map.values():
            x = set([x for x in colors if colors.count(x) > 1])
    ...
    def reward(self, is_valid_move, is_old_observation, hanging_edges_status, cont, moves_possibles):
        ...
        if len(self.possible_moves) == 0 or len(moves_possibles) == 0 or self.cont_invalid > 10:
            return -100 * self.element_number
    ...
    def _is_valid_move(self, action_index) -> bool:
        ...
        for possible_move in self.possible_moves:
            ...
            if first_condition or second_condition:
                ...
                if first_condition or second_condition:
                    observation = self._get_obs()
                    is_old_observation = any(all(x == observation) for x in self.prev_observation) # MlpPolicy
                    if not is_old_observation:
                        return True, []
                    else:
                        self._unmove(possible_move)
                else:
    ...
    def step(self, action):
        self.possible_moves, self.hanging_edges_status, cont = self._get_possible_moves()
        ...
        if is_valid_move:
            ...
            observation = self._get_obs()
            is_old_observation = any(all(x == observation) for x in self.prev_observation) # MlpPolicy
            # is_old_observation = any(all(x == observation['trails']) for x in self.prev_observation) # MultiInputPolicy
            self.prev_observation.append(observation)
            self.possible_moves, self.hanging_edges_status, cont = self._get_possible_moves()
        else:
            self.cont_invalid += 1
            observation = self.prev_observation[-1]
        ...
        reward = self.reward(is_valid_move, is_old_observation, self.hanging_edges_status, cont, [None])
        return observation, reward, self.terminal(self.possible_moves, self.hanging_edges_status, is_old_observation), False, {}
    ...
    def terminal(self, possible_moves, hanging_edges_status, is_old_observation):
        if hanging_edges_status:
            ...
            print("GANHOU!")
            print("*"*100)
        return hanging_edges_status or len(possible_moves) == 0 or is_old_observation or self.cont_invalid > 10
