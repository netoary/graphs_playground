...
from rl.env import RegularDecompositionEnvironment
from brute.path_decomposition import PathDecomposition
from rl.utils import canonicalDecomposition
from sb3_contrib import MaskablePPO

class RLPathDecompositionMothod(PathDecomposition):
    def __init__(self, G) -> None:
        self._graph = Graph(G)
        self._start_graph = Graph(G)
        self._n = self._graph.order()
        models_dir = f"rl/models"
        # self.model_path = f"{models_dir}/3490000_planeres"
        self.model_path = f"{models_dir}/1940000_16_planar"

    def _load_model(self, H):
        self.env = RegularDecompositionEnvironment(5, H)
