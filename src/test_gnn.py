from gnn.gnn_method import GNNPathDecompositionMothod
...
class TestGnn(TestAbstract):
    ...
    def run_test(self, path, test_name, single_matching = False, step = 10):
        save_path = f'../outputs/gnn_{test_name}.csv'
        with open(path, 'r') as graphs_file:
            ...
            for x in graphs_file:
                ...
                print(x)
                graph = Graph(x)
                rl = GNNPathDecompositionMothod(graph)
                if single_matching:
                    rl_dic.append(rl.run_simgle_matching())
                else:
...
# t.run_all()
# t.run_bipartite()
t.run_big_benchmark(1)
# t.run_extra_big_benchmark(1)
